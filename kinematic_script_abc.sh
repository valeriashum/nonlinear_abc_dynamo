...
set_temp_file_and_run () {
	...
	cp src/problem/kinematic_ABC_flow/gvars.h    src/problem/kinematic_ABC_flow_temp/
	cp src/problem/kinematic_ABC_flow/snoopy.cfg src/problem/kinematic_ABC_flow_temp/
...
for i in `seq 1 3`; do
    echo "Rm = $reynolds_m"
    set_temp_file_and_run
    reynolds_m=$(echo "$reynolds_m + 5.0" | bc)
done
