...
int isComment(struct dataLine *lineIn){
	int i;
	for(i=0; i < lineIn->recordPos[lineIn->numRecords]; i++)
		if(lineIn->charArray[i] != ' '){
			if(lineIn->charArray[i] == TSV_COMMENT_CHAR) return 1;
			else return 0;	// Found a non-whitespace character that was not a comment
		}
	
	return 2;	// The entire line was whitespace
...
int gobbleLine(FILE *stream, struct dataLine *lineOut){
	...
	if(char1 == EOF) return 1;	// Let us know if we hit the end of the file
...
int dataLineToFloats(struct dataLine *lineIn, struct floatRecords *floatsOut){
	...
	floatsOut->floatArray[0] = strtod(recordNullTerminated, NULL);
	for(i=1; i<floatsOut->numRecords; i++){	// Loop over records
		...
		recordNullTerminated[j] = '\0';
		floatsOut->floatArray[i] = strtod(recordNullTerminated, NULL);
	}
